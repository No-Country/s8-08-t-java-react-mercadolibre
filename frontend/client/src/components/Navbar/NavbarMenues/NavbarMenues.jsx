    ...
    <div className="hidden sm:flex justify-between items-center w-full gap-4">
      <div className="flex gap-2 items-center font-light">
        ...
        <div className="flex flex-col">
          <p className="opacity-60 hover:opacity-90 cursor-pointer text-[0.8rem]">Enviar a Dario</p>

          <p className="font-medium whitespace-nowrap">Esteban Quito 540</p>
        </div>
      </div>
      <ul className="flex items-center gap-4 font-light text-sm relative">
        {menu.map((item, i) => (
          <li
            key={`${i}-menu`}
          ...
          >
            {item.name && <p className="opacity-60 hover:opacity-90 cursor-pointer">{item.name}</p>}
            {item?.icon() && item.icon()}
          </li>
        ))}
        {openMenuCategory && (
